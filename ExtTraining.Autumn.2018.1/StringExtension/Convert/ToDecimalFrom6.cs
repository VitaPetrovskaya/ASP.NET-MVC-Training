...
namespace StringExtension
{
     ...
     /// </summary>
     public class ToDecimalFrom6 : IConvertNumber
     {
          /// <summary>
          ...
          public int ConvertToDecimal(string str)
          {
               checked
               {
                    ...
                    for (int i = 0; i < array.Length; i++)
                    {
                         if (array[i] == '1')
                         {
                              result += (int)Math.Pow(6, i);
                         }

                         if (array[i] == '2')
                         {
                              result += 2 * (int)Math.Pow(6, i);
                         }

                         if (array[i] == '3')
                         {
                              result += 3 * (int)Math.Pow(6, i);
                         }

                         if (array[i] == '4')
                         {
                              result += 4 * (int)Math.Pow(6, i);
                         }

                         if (array[i] == '5')
                         {
                              result += 5 * (int)Math.Pow(6, i);
                         }
                    }
